...
function App() {
  ...
  const notwatched = (data,key) =>{
    return(
      <ListGroup.Item className="lstcls" onClick={event=>{handleClick(key)}} key={key}> <ToggleButton
              ...
              >
                MARK AS WATCHED
              </ToggleButton>{data.title} <BsDot style={{float:"right",color:"red",size:"20px"}}/></ListGroup.Item>
    );
  }
  ...
  const notwatchedd = (data,key) =>{
    return(
      <ListGroup.Item className="lstcls" onClick={event=>{handleClick(key)}} key={key}> <ToggleButton
              ...
              >
                MARK AS WATCHED
              </ToggleButton>{data.title} </ListGroup.Item>
    );
  }
    ...
    <div className="App">
      <Navbar bg="dark" variant="dark">
        <Container>
          ...
          <Nav className="ml-auto">
            <Nav.Link href="https://www.parivartancomputing.in/home">Home</Nav.Link>
            <Nav.Link href="https://www.parivartancomputing.in/about-us">About Us</Nav.Link>
            <Nav.Link href="https://www.parivartancomputing.in/courses">Courses</Nav.Link>
          </Nav>
        </Container>
      </Navbar>
      <Container fluid style={{height:"100%"}}>
        <Row style={{height:"100%"}}>
          <Col lg={8} xs={12}>
             <iframe style={{marginTop:"10%"}} width="80%" height="60%" src={courseData[idx].url} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
          </Col>
          <Col>
            {listdata()}
          </Col>
